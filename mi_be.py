...
#generation du mi_be
    # x = np.arange(160000)
    # note_mi_be = np.zeros(len(fft_signal))
    # for i in range(33):
    #     note_mi_be += harmonique_amplitude[i] * np.sin(2 * np.pi * ((harmonique_frequence[i]/1.06)*0.707 / sample_rate) * x + harmonique_phase[i])
    #
    # mi_be_synthese = enveloppe * note_mi_be
    # mi_be_synthese = sol_synthese * 32000 / np.max(np.abs(mi_be_synthese))  # normalise la sythese audio
    #
